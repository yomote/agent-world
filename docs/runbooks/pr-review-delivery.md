...
固定campaign外の通常PM運用では`python -m scripts.automation.review_delivery_cli`を使う。`initial`、`final`と各`reconcile-*`だけを提供し、専用SQLiteへfrozen input、head、stage、前段/最終receiptを保存する。campaignのreal job完了を偽装せず、既存の型検査、postcondition検査、GitHub adapterをそのまま再利用する。結果不明時は対応する`reconcile-*`以外を実行せず、別headは同DBの別履歴として保持する。
