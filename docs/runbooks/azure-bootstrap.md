...
auth、immutable構成、smokeまで成功した後の`external-after.private.json`保存だけが失敗した場合、公開自体は成功として終了し、再公開しない。warningを記録し、read-only actualからlocal evidenceだけを後で修復する。公開前snapshotを保存できない場合はwrite前に停止する。
