...
新しい業務デモ候補を選定して初回実装するwork itemと、既存デモの目的・価値を実質的に変えるwork itemは、実装前に短い課題提案をPOへ示します。提案には、誰がどの場面で困るか、現在の対応と負担、何を肩代わりするか、代表的な入力から結果までの1例、人がどう楽になるか、通常のcodeで担う部分とAIで試す仮説を含めます。wireや操作説明だけで課題・価値の説明を代替せず、POとその認識を合わせてから着工します。すでに課題・価値を合意済みのデモの保守と機械的な変更は、この着工条件の対象外です。

AI仮説や価値が未確認なら実証済みと扱いません。弱い候補は棄却でき、棄却や認識不一致を実装開始へ読み替えません。
