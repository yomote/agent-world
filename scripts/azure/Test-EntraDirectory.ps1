...
foreach ($principalId in $AllowedPrincipalObjectIds) {
  ...
  $assignmentsJson = & az rest --only-show-errors --method get --uri "https://graph.microsoft.com/v1.0/users/$principalId/appRoleAssignments" --query 'value[].{resourceId:resourceId,principalId:principalId}' --output json
