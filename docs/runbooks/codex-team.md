...
2026-09-05 の導入セッションでは、実行枠は親を含めて 4 と観測しました。これは普遍値ではありません。設定の `max_concurrent_threads_per_session = 3` は子の上限であり、枠の増加を保証しません。通常は 2〜3 子で十分です。委任のためだけに細分化・無理な並列化をせず、具体的に独立した作業があるときだけ分配します。
