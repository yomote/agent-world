...
> [!NOTE]
> **経理担当の困り事:** 「消込」とは、銀行へ入ったお金を未払いの請求へ対応づける作業である。デモでは資料と未解決事項を確認し、`資料・条件を選ぶ → 調査開始 → 必要なら質問へ回答 → 根拠付き結果を確認`の順で、判明した対応、未確認、次の担当へ渡す成果物を見る。これは実装予定の初期説明boxであり、現在のUIに実装済みではない。

初見の利用者には上の説明と操作をtraceより先に表示し、詳細を開いたときだけ次を示す。

- **渡すもの:** 合成した銀行明細、請求残高、送金通知、メール、調整履歴。Agentが解けない項目は隠さず未配分・未回収一覧と営業確認票へ残す。
- **受け取るもの:** 入金と請求の対応案、参照した資料と箇所、判明/未確認の区別、次担当と確認事項を含むレビューpackage。
- **成功/失敗の公開説明例:** 70,000円の請求へ50,000円が入金され、過去の20,000円値引きが取消済みなら、全額支払済みにしない。50,000円の配分案と20,000円の未回収を根拠付きで渡せれば成功、取消済み値引きを足して完済扱いにすれば失敗。この例は説明用であり、Agentから隔離するhidden評価caseとは別にする。
- **表示上の境界:** 合成データ、簡略化した業務、台帳を更新しない提案、実モデルを実行した結果か説明用sampleかを常に表示する。

体験の受入条件は、非専門家が前説明と成果物だけから「誰の何の困り事か」「何を発見したか」「どの根拠を使ったか」「誰が次に何へ使うか」を説明できることである。domain判断の正しさを非専門家へ採点させない。

実務妥当性とAgentの追加価値は別の評価書で扱う。同じ資料、tool、権限、人回答、solverを固定workflowとAgentへ与え、誤配分、適切な保留、人の修正・質問負担、所要時間、費用を比較する。現時点ではどちらも未実証である。
